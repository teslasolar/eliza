...
**L1 Authority (PoA):** Permanent record. Immutable. Refusals, certs, membership.
Raspberry Pi capable. 60s blocks. 5-11 authority nodes.

**L2 Operations (DPoS):** Real-time ops. Fast. Vetting sessions, PackML state, events.
...
Once a refusal is recorded here, it cannot be altered, deleted, or suppressed.
Every guild member should be able to run a node. Raspberry Pi is the floor.
...
The guild's nervous system. Fast, cheap, operational.
Important data gets checkpointed to L1 for permanence.
...
Ethical refusals bypass the checkpoint queue — immediate L1 write.
