"""
Guild Agents — the 8 vertex agents remapped from KCC cube architecture.

KCC's 8 vertex agents become 6 guild unit agents + 2 system agents.
512 sub-cubes become operation slots.
"""
...
class GuildAgent:
    ...
    role: AgentRole
    vertex: CubeVertex
    kcc_original: str           # what this agent did in KCC
    guild_function: str         # what it does in ACG-KCC
    accuracy: str = ""
    slot_range: str = ""        # operation slots owned
...
class OperationSlot:
    ...
    slot_id: int
    agent_team_size: int = 4        # mini-agent team per slot
    packml_states: int = 17         # full PackML state machine
    active: bool = False
    operation_type: str = ""

    @property
    def owning_unit(self) -> str:
        """Which unit owns this slot based on ID range."""
        if self.slot_id < 128:
            return "VETTING"
