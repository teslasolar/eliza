# API Server — Lightweight REST Interface

This module provides a REST-style HTTP server for konomi directory operations, built entirely on Python's standard library (`http.server`). It exposes endpoints for listing packages, reading and searching tags, compiling to L5X, and retrieving a system summary. The `_Handler` class delegates all business logic to `DirectoryOps`, keeping the HTTP layer thin.
...
## APIServer Class and Entry Point
