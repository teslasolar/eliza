...
This module provides a command-line interface that wraps `DirectoryOps` with argument parsing and formatted output. It supports subcommands for listing packages, reading tags, searching, compiling to L5X, and printing a system summary. All output is either plain text (for lists) or JSON (for structured data), and error handling is uniform through the `OpResult` pattern.

## Argument Parser and Dispatch
