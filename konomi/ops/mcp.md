# MCP Provider — Model Context Protocol Tool Definitions

This module exposes konomi directory operations as MCP tools that any MCP-compatible client (Claude Code, Cursor, etc.) can discover and invoke. Each tool maps one-to-one to a `DirectoryOps` method. Tool definitions are expressed as frozen dataclasses (`MCPToolDef`, `MCPToolParam`) that can serialize themselves to JSON Schema, and the `MCPProvider` class handles both tool listing and tool execution with standardized result formatting.

## Tool Parameter and Definition Dataclasses
...
## MCPProvider — Tool Registry and Dispatch
