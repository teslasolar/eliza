# L5X Rungs and Routines — Ladder Logic Structure

This module defines the structural elements of ladder logic programs. A `Rung` is a single line of logic consisting of input conditions wired to output instructions. A `LadderRoutine` is a named sequence of rungs that lives inside a program.

The `Rung` class provides chainable shorthand methods (`.xic()`, `.ote()`, `.ton()`, etc.) so you can build rungs fluently. Each rung can serialize itself to both plain-text and XML formats suitable for L5X export.

## RungElement — A Single Instruction on a Rung
...
## Rung — A Single Line of Ladder Logic
...
## LadderRoutine — A Named Sequence of Rungs
